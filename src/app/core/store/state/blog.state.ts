...
import {
  ...
  BlogPost_Defaults,
  Blog_Defaults,
} from 'src/app/shared/model/blog';
...
export class BlogState {
  ...
  getBlogPost(
    ...
    setState({
      ...state,
      blogPost: state.blogPosts.filter((data) => {
        return data.id.toString() === action.payload;
      })[0],
    });
  }
        ...
        patchState({
          blogPosts: [
            ...state.blogPosts.filter(
              (value: BlogPost) => value.id !== action.payload.id
            ),
            action.payload,
          ],
        });
      ...
      tap((result: any) => {
        ...
        patchState({
          blogPosts: state.blogPosts.filter(
            (value: BlogPost) => value.id !== action.payload
          ),
        });
      })
    );
